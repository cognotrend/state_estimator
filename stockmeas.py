...
class StockMeasurement():
    def __init__(self,
        ...
        if self.subfilters==False:
        ...
        else:  # don't believe this branch has been excuted yet, since composite in hier2.py is a pseudo-composite.
            ...
            for dummy in range(self.subfilters_list[0].numruns):
    ...
    def nextMeas(self):
        ...
        if self.subfilters==False:
            if self.logmode==1:
                ...
                ref_meas = np.log(0.5*(self.dfs[0].loc[next_tstamp]['open']+self.dfs[0].loc[next_tstamp]['close']))
            else:
                ref_meas = 0.5*(self.dfs[0].loc[next_tstamp]['open']+self.dfs[0].loc[next_tstamp]['close'])
            if self.num_stocks>1:
                for i in list(range(0,self.num_stocks-1)):
                    if self.logmode==1:
                        self.meas_array[i]= np.log(0.5*(self.dfs[0].loc[next_tstamp]['open']+self.dfs[0].loc[next_tstamp]['close'])) - ref_meas
                    else:
                        self.meas_array[i]= 0.5*(self.dfs[0].loc[next_tstamp]['open']+self.dfs[0].loc[next_tstamp]['close']) - ref_meas
